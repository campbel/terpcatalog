        ...
        <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
          ...
          <div v-for="strain in strains" :key="strain.id" class="group relative">
            ...
            <div v-else class="flex">
              <select class="mr-4 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 text-center">
                <option v-for="num in 10" :key="num" :value="num">{{ num }}</option>
              </select>
              <button class="bg-gray-200 hover:bg-gray-400 rounded-md px-3 py-2">Remove</button>
            </div>
          </div>
